...
public class frontbooking {
    ...
    public String getTrip(Model model, @PathVariable("id") int tripFilterId,@ModelAttribute("chosenSeat")int chosenSeat,RedirectAttributes redirect) {
        ...
        Customer customer;
        customer=customerService.findByUsername(currentLoggedinUser);
	...
	/*
	 * @RequestMapping("/customer/{'cusName'}") public String getCustomer(Model
	 * model,@Param("cusName")String cusName){ Customer
	 * customer=customerService.findByusername(cusName);
	 * model.addAttribute("customer",customer);
	 * 
	 * return "frontend/booking"; }
	 */
